...
Enable the `alpha` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["alpha"] }
```

In your code, import the `alpha` client:
...
- **Official Binance Alpha API docs:** [Binance API Documentation](https://developers.binance.com/docs/alpha)
...
The `alpha` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
To run the tests for the Alpha module:

```bash
cargo test --features alpha
```
