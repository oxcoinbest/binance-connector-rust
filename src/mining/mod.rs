...
use crate::common::{
    config::ConfigurationRestApi, constants::MINING_REST_API_PROD_URL, utils::build_user_agent,
};
...
impl MiningRestApi {
    ...
    ///
    /// If no base path is specified in the configuration, defaults to the production Mining REST API URL.
    ///
    /// # Arguments
    ///
    ...
    pub fn from_config(mut config: ConfigurationRestApi) -> rest_api::RestApi {
        config.user_agent = build_user_agent("mining");
        if config.base_path.is_none() {
            config.base_path = Some(MINING_REST_API_PROD_URL.to_string());
        }
    ...
    pub fn production(mut config: ConfigurationRestApi) -> rest_api::RestApi {
        config.base_path = Some(MINING_REST_API_PROD_URL.to_string());
        MiningRestApi::from_config(config)
    }
}
