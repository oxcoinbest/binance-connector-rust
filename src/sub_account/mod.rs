...
pub struct SubAccountRestApi {}

impl SubAccountRestApi {
    /// Creates a REST API client with the given configuration.
    ///
    /// If no base path is specified in the configuration, defaults to the production `SubAccount` REST API URL.
    ///
    /// # Arguments
    ///
    ...
    pub fn production(mut config: ConfigurationRestApi) -> rest_api::RestApi {
        config.base_path = Some(SUB_ACCOUNT_REST_API_PROD_URL.to_string());
        SubAccountRestApi::from_config(config)
    }
}
