...
This module provides the official Rust client for Binance's NFT API, enabling developers to interact programmatically with Binance's NFT trading platform. The library provides tools to launch and trade NFTs through the REST API:
...
Enable the `nft` feature in your `Cargo.toml` under `binance-sdk`:

```toml
[dependencies]
binance-sdk = { version = "1.0.0", features = ["nft"] }
```

In your code, import the `nft` client:
...
- **Official Binance NFT API docs:** [Binance API Documentation](https://developers.binance.com/docs/nft)
...
The `nft` module can be configured with the following options via the `ConfigurationRestApi` builder pattern:
...
To run the tests for the NFT module:
...
If you are upgrading from a legacy, single-crate connector that included NFT support, see the [Migration Guide](../../MIGRATION.md) for instructions on enabling the `nft` feature.
...
Please ensure that all tests pass with `--features nft` if you're making a direct contribution. Submit a pull request only after discussing and confirming the change.
