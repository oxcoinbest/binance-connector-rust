...
use crate::common::{
    config::ConfigurationRestApi, constants::VIP_LOAN_REST_API_PROD_URL, utils::build_user_agent,
};

/// Represents the `VIPLoan` REST API client for interacting with the Binance `VIPLoan` REST API.
///
/// This struct provides methods to create REST API clients for production  environments.
pub struct VIPLoanRestApi {}

impl VIPLoanRestApi {
    /// Creates a REST API client with the given configuration.
    ///
    /// If no base path is specified in the configuration, defaults to the production `VIPLoan` REST API URL.
    ///
    /// # Arguments
    ///
    ...
    pub fn from_config(mut config: ConfigurationRestApi) -> rest_api::RestApi {
        config.user_agent = build_user_agent("vip-loan");
    ...
    pub fn production(mut config: ConfigurationRestApi) -> rest_api::RestApi {
        config.base_path = Some(VIP_LOAN_REST_API_PROD_URL.to_string());
        VIPLoanRestApi::from_config(config)
    }
}
