...
use binance_sdk::derivatives_trading_coin_futures::{
    DerivativesTradingCoinFuturesWsStreams, websocket_streams::KlineCandlestickStreamsParams,
};
...
async fn main() -> Result<()> {
    ...
    let params =
        KlineCandlestickStreamsParams::builder("btcusdt".to_string(), "1m".to_string()).build()?;
