...
async fn main() -> Result<()> {
    ...
    let response = rest_client
        ...
        .await
        .context("get_margin_restricted_assets request failed")?;

    info!(?response.rate_limits, "get_margin_restricted_assets rate limits");
    let data = response.data().await?;
    info!(?data, "get_margin_restricted_assets data");
