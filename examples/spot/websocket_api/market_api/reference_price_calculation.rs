...
use binance_sdk::spot::{SpotWsApi, websocket_api::ReferencePriceCalculationParams};
...
async fn main() -> Result<()> {
    ...
    let params = ReferencePriceCalculationParams::builder("BNBUSDT".to_string()).build()?;

    // Make the WS API call
    let response = connection
        .reference_price_calculation(params)
        .await
        .context("reference_price_calculation request failed")?;

    info!(?response.rate_limits, "reference_price_calculation rate limits");
    let data = response.data()?;
    info!(?data, "reference_price_calculation data");
